...
import ca.chaves.androidApp.familyBrowser.database.NodeController;
import ca.chaves.androidApp.familyBrowser.database.NodeDatabase;
...
/**
 * The class FamilyController implements the controller for the FamilyBrowser
 * main screen. It handles the user interaction and the database access. *
 * 
 * @author david@chaves.ca
 */
public class FamilyController extends NodeController {
    
    /**
     * The main view to update. The value m_userView is set when the screen is
     * fully built, and it is used to force a redraw once the user-interface
     * data buffer changes.
     */
    private View m_userView;
    /**
     * The list of TextView to update when the user-interface data buffer
     * changes. These TextView(s) display the current NodeObject name.
     */
    private final LinkedList<TextView> m_textViews = new LinkedList<TextView>();
    /**
     * The list of ListView to update when the user-interface data buffer
     * changes. These ListView(s) display the node attributes or the node
     ...
     * @param activity
     */
    public FamilyController(final Activity activity) {
    ...
    public void onTabIndexChildClick(final int groupPosition, final int childPosition) {
        ...
        final Integer id = list.getExternalId(childPosition);
        final String name = list.getStringValue(childPosition);
    ...
    protected void onLoadCompleteRefreshUserInterface(final NodeObject node) {
        ...
        for (ListView listView : this.m_listViews) {
            ...
            // FamilyController to populate them
